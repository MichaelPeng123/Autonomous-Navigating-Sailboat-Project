/* RCPassthrough rev 7/30/2017
© 2014-2017 RoboSail
Find detailed description in Decription tab
*/
#include <Servo.h>
#include "RoboSail.h"
// ****** Find the values for the specific transmitter (using program RCReader)
// ****** and change the values below as needed
#define SAIL_LOW 1090
#define SAIL_HIGH 1900
#define RUDDER_LOW 1000
#define RUDDER_HIGH 2000
...
void loop() {
  ...
  rudderServoOut = map(rudderPulseWidth, RUDDER_LOW, RUDDER_HIGH, -60, 60);

  sailPulseWidth = pulseIn(SAIL_RC_PIN, HIGH);
  sailServoOut = map(sailPulseWidth, SAIL_LOW, SAIL_HIGH, 0, 90);
  ...
  // Adjust the servo values from the desired angles  
